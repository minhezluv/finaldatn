  ...
  <div class="content">
    ...
    <div class="content__main">
      <ContentUtility
        ...
        @contractCurrent="contractCombobox"
        @statusCurrent="statusCombobox"
        @deleteSelectedRows="
    ...
    <BasePopup v-show="showDeletePopup">
      <template #content>
        ...
        <div class="popup__text">
          Bạn có thực sự muốn xóa Hợp đồng {{ deleteString }} không?
        </div>
      </template>
      <template #footer>
        <div
      ...
      <template #content>
        ...
        <div class="popup__text">
          ...
          <span class="text-semibold">{{
            LCTable.currentSelectedRows.length
          }}</span>
          Hợp đồng không?
        </div>
      </template>
...
import LCAPI from "../../api/components/laborcontracts/LaborContractsAPI";
...
export default {
    ...
    return {
      LCTable: {
        column: [
          ...
          {
            columnName: "Họ và Tên nhân viên",
            fieldName: "staffName",
          },
          {
          ...
          {
            columnName: "Số bảo hiểm y tế",
            fieldName: "healthICode",
          },
          {
            columnName: "Tên số bảo hiểm xã hội",
            fieldName: "socialICode",
          },
          {
            columnName: "Loại hợp đồng ",
            fieldName: "typeLaborContractID",
            dataType: Resource.DataTypeColumn.Enum,
            enumName: "TypeLaborContract",
          },
          {
            columnName: "Trạng thái hợp đồng",
            fieldName: "status",
            dataType: Resource.DataTypeColumn.Enum,
            enumName: "WorkStatus",
          },
  ...
  methods: {
    ...
    async getDataServer() {
      ...
      await LCAPI.filter(
        this.LCTable.pageSize,
        this.LCTable.currentPageNum,
        this.LCTable.filterValue,
        this.LCTable.departmentID,
        this.LCTable.positionID,
        this.LCTable.typeLaborContractID,
        this.LCTable.status
      )
        .then((response) => {
          console.log(response.data);
          this.LCTable.gridData = response.data.data;
          this.LCTable.totalRecord = response.data.infoPage.totalRecord;
          this.LCTable.totalPage = response.data.infoPage.totalPage;
        })
        .catch(() => {
          this.showErrorPopup = true;
          this.LCTable.gridData = [];
    ...
    async departmentCombobox(val, id) {
      this.LCTable.departmentID = id;
      this.LCTable.currentPageNum = 1;
      console.log("departmentID: ", val, id);
      this.getDataServer();
    },
    /**
     * Lay id chuc danh
    ...
    async positionCombobox(val, id) {
      this.LCTable.positionID = id;
      this.LCTable.currentPageNum = 1;
      console.log("positionID: ", val);
      this.getDataServer();
    },

    async contractCombobox(val, id) {
      this.LCTable.typeLaborContractID = id;
      this.LCTable.currentPageNum = 1;
      console.log("positionID: ", id);
      this.getDataServer();
    },
    async statusCombobox(val, id) {
      this.LCTable.status = id;
      this.LCTable.currentPageNum = 1;
      console.log("positionID: ", id);
      this.getDataServer();
    },
    ...
    clickFunctionItem(fn, item) {
      switch (fn) {
        case "Xóa":
          this.deleteString = "<" + item.laborContractCode + ">";
    ...
    async deleteSelectedEmployee() {
      await LCAPI.delete(this.employeeDelete.guid)
        .then(async (response) => {
    ...
    async getDataFilter(filterValue) {
      this.LCTable.filterValue = filterValue;
      this.LCTable.currentPageNum = 1;
    ...
    changePageSize(pageSize) {
      this.LCTable.pageSize = pageSize;
    ...
    changePageNum(pageNum) {
      this.LCTable.currentPageNum = pageNum;
      this.getDataServer();
    },
     ...
     */
    exportData() {
      LCAPI.exportData(this.LCTable.filterValue)
        .then((response) => {
          if (response) {
    ...
    changeListSelectedRow(list) {
      this.LCTable.currentSelectedRows = JSON.parse(JSON.stringify(list));
    },
    ...
    openMultipleDeletePopup() {
      ...
      if (this.LCTable.currentSelectedRows.length == 1) {
        this.employeeDelete =
          this.LCTable.gridData[this.LCTable.currentSelectedRows[0]];

        this.deleteString = "<" + this.employeeDelete.laborContractCode + ">";
    ...
    async deleteSelectedRows() {
      ...
      this.LCTable.currentSelectedRows.map((item) => {
        listId.push(this.LCTable.gridData[item].employeeId);
      });

      //Xóa
      await LCAPI.multipleDelete(listId)
        .then(async (response) => {
          if (response.status != 204) {
            //Tắt popup

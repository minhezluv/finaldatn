...
var TkYear = [
  {
    ...
    headerTitle: "Thống kê tổng số nhân viên",
    lineChartData: {
      labels: [
        "Jan",
        "Feb",
...
export default {
  ...
  methods: {
    async generateWeek() {
      ...
      this.customWeekData.defaultValue = "9";
    },
    ...
    async getByWeekOption() {
      await TK.getByWeekOption(this.curryear, this.currweek)
        .then((response) => {
          ...
          this.dataChart[0].barChartData.datasets[0].data =
            response.data.allwork;
          console.log(this.dataChart[0].barChartData.datasets[0].data);
          this.IsBarLoad = true;
        })
        .catch((e) => {
    ...
    async getByYearAll() {
      await EmployeesAPI.GetAllStaffPerMonth(this.curryear2)
        .then((response) => {
